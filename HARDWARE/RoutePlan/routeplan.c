...
extern  long int Line_Turn_Flag ;
extern long int Line_Turn_Time_Out;
...
void CoordinatePositionMovement(float CurrentX, float CurrentY, float ExpectedX, float ExpectedY){
	...
	#else
	motor_circle_angle[0] =  (errX + errX / fabs(errX) * Length_Speed_K * LINEAR_SPEED * LINEAR_SPEED)/(PI*WHEEL_DIAMETER)*360 ;
	motor_circle_angle[1] = -(errY + errY / fabs(errY) * Length_Speed_Ky * LINEAR_SPEED * LINEAR_SPEED )/(PI*WHEEL_DIAMETER)*360 ;
	motor_circle_angle[2] = -motor_circle_angle[0];
	motor_circle_angle[3] = -motor_circle_angle[1];
	#endif
	for(i=0;i<4;i++){
...
void routeplan(int s, int e, int startORend){
	...
	*/
	...
	if(startORend == 1){
		...
		if(s==1&&e==0){
			CoordinatePositionMovement(motor_Position[0], motor_Position[1], CTransX(-2000.00f, 0.00f), CTransY(-2000.00f, 00.00f));
			AngularRotationMovement(motor_Position[2], -180.0f);
		...
		if(s==2&&e==2){
			CoordinatePositionMovement(motor_Position[0], motor_Position[1], CTransX(-2000.00f, 0.00f), CTransY(-2000.00f, 0.00f));
			AngularRotationMovement(motor_Position[2], 180.0f);
		}
...
int sign(float a){
 ...
 void Line_Turn(float Vexpx ,float Vexpy,float Angle_Speed) // m/s   m/s   rad/s
{
	float Afera = 0;
	float Vx = 0;
	float Vy = 0;
	Line_Turn_Time_Out = INT32_C(PI / Angle_Speed * 1000);
	//Line_Turn_Time_Out = 10000;
	Vexpx = 1000 * Vexpx  ;    //mm/s
	Vexpy = 1000 * Vexpy  ;    //mm/s
	float k = 1;
	Vexpx = Vexpx / PI / WHEEL_DIAMETER * 60*k ;  //rpm
	Vexpy = Vexpy / PI / WHEEL_DIAMETER * 60*k ;  //rpm
	//Angle_Speed = Angle_Speed / 2 / PI * 60 ;    //rpm
	
	Line_Turn_Flag = 0;
	do
	{
	Afera = Line_Turn_Flag / 1000 * Angle_Speed;
	Vx =  Vexpx * cos(Afera) + Vexpy * sin(Afera);
	Vy = -Vexpx * sin(Afera) + Vexpy * cos(Afera); 
	Speed_Disassemble(Vx,Vy,Angle_Speed);
	}while (Line_Turn_Flag < Line_Turn_Time_Out);
	MotorSpeedExpected(0,0,0,0,0);
}

void Speed_Disassemble(float Vx,float Vy , float Angle)
{
	Angle = Angle  * MOTOR_DIAMETER / WHEEL_DIAMETER; //rad/s
	Angle = Angle / 2 / PI * 60 ; //rpm
	Motor_1.State = PIDSPEED;
	Motor_2.State = PIDSPEED;
	Motor_3.State = PIDSPEED;
	Motor_4.State = PIDSPEED;
	Motor_1.SpeedExpected =  Vx * wzy - Vy * wzy - Angle;
	Motor_2.SpeedExpected = -Vx * wzy - Vy * wzy - Angle;
	Motor_3.SpeedExpected = -Vx * wzy + Vy * wzy - Angle;
 	Motor_4.SpeedExpected =  Vx * wzy + Vy * wzy - Angle;
}
